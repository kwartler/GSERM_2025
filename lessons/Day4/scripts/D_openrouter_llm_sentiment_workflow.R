...
headers <- c(
  ...
  `X-Title`       = "R Aspect Sentiment Script"
)
...
# Organize Request 1: Attribute Extraction
...
# Make the first POST request to OpenRouter
...
# Organize Request 2: Sentiment Assignment
dataLLM <- list(model = llmModel,
                messages = list(
...
# Make the second POST request to OpenRouter
res <- httr::POST(url = "https://openrouter.ai/api/v1/chat/completions",
                  httr::add_headers(.headers = headers),
                  body = toJSON(dataLLM, auto_unbox = TRUE))

# Extract the final response
llmResponseFinal <- httr::content(res)$choices[[1]]$message$content
cat(llmResponseFinal)
